...
foreign box2d
{
	...
	@(link_name="b2MinFloat")
	min_float :: proc(a: f32, b: f32) -> f32 ---

	// @return the maximum of two floats
	@(link_name="b2MaxFloat")
	max_float :: proc(a: f32, b: f32) -> f32 ---

	// @return the absolute value of a float
	@(link_name="b2AbsFloat")
	abs_float :: proc(a: f32) -> f32 ---

	// @return a float clamped between a lower and upper bound
	@(link_name="b2ClampFloat")
	clamp_float :: proc(a: f32, lower: f32, upper: f32) -> f32 ---
	...
	@(link_name="b2AbsInt")
	abs_int :: proc(a: int) -> int ---

	// @return an integer clamped between a lower and upper bound
	@(link_name="b2ClampInt")
	clamp_int :: proc(a: int, lower: int, upper: int) -> int ---
	...
	@(link_name="b2CrossVS")
	cross_vs :: proc(v: Vec2, s: f32) -> Vec2 ---

	// Perform the cross product on a scalar and a vector. In 2D this produces a vector.
	@(link_name="b2CrossSV")
	cross_sv :: proc(s: f32, v: Vec2) -> Vec2 ---

	// Get a left pointing perpendicular vector. Equivalent to b2CrossSV(1.0f, v)
	@(link_name="b2LeftPerp")
	left_perp :: proc(v: Vec2) -> Vec2 ---

	// Get a right pointing perpendicular vector. Equivalent to b2CrossVS(v, 1.0f)
	@(link_name="b2RightPerp")
	right_perp :: proc(v: Vec2) -> Vec2 ---
	...
	@(link_name="b2MulSV")
	mul_sv :: proc(s: f32, v: Vec2) -> Vec2 ---

	// a + s * b
	@(link_name="b2MulAdd")
	mul_add :: proc(a: Vec2, s: f32, b: Vec2) -> Vec2 ---

	// a - s * b
	@(link_name="b2MulSub")
	mul_sub :: proc(a: Vec2, s: f32, b: Vec2) -> Vec2 ---
	...
	@(link_name="b2LengthSquared")
	length_squared :: proc(v: Vec2) -> f32 ---

	// Get the distance between two points
	@(link_name="b2Distance")
	distance :: proc(a: Vec2, b: Vec2) -> f32 ---

	// Get the distance squared between points
	@(link_name="b2DistanceSquared")
	distance_squared :: proc(a: Vec2, b: Vec2) -> f32 ---
	...
	// Normalize rotation
	@(link_name="b2NormalizeRot")
	normalize_rot :: proc(q: Rot) -> Rot ---

	// Is this rotation normalized?
	@(link_name="b2IsNormalized")
	is_normalized :: proc(q: Rot) -> bool ---

	// Normalized linear interpolation
	// https://fgiesen.wordpress.com/2012/08/15/linear-interpolation-past-present-and-future/
	@(link_name="b2NLerp")
	n_lerp :: proc(q1: Rot, q2: Rot, t: f32) -> Rot ---

	// Integration rotation from angular velocity
	//	@param q1 initial rotation
	//	@param deltaAngle the angular displacement in radians
	@(link_name="b2IntegrateRotation")
	integrate_rotation :: proc(q1: Rot, deltaAngle: f32) -> Rot ---

	// Compute the angular velocity necessary to rotate between two rotations over a give time
	//	@param q1 initial rotation
	//	@param q2 final rotation
	//	@param inv_h inverse time step
	@(link_name="b2ComputeAngularVelocity")
	compute_angular_velocity :: proc(q1: Rot, q2: Rot, inv_h: f32) -> f32 ---
	...
	@(link_name="b2TransformPoint")
	transform_point :: proc(t: Transform, p: Vec2) -> Vec2 ---

	// v2 = A.q.Rot(B.q.Rot(v1) + B.p) + A.p
	//    = (A.q * B.q).Rot(v1) + A.q.Rot(B.p) + A.p
	@(link_name="b2MulTransforms")
	mul_transforms :: proc(A: Transform, B: Transform) -> Transform ---

	// v2 = A.q' * (B.q * v1 + B.p - A.p)
	//    = A.q' * B.q * v1 + A.q' * (B.p - A.p)
	@(link_name="b2InvMulTransforms")
	inv_mul_transforms :: proc(A: Transform, B: Transform) -> Transform ---

	// Multiply a 2-by-2 matrix times a 2D vector
	@(link_name="b2MulMV")
	mul_mv :: proc(A: Mat22, v: Vec2) -> Vec2 ---

	// Get the inverse of a 2-by-2 matrix
	@(link_name="b2GetInverse22")
	get_inverse_22 :: proc(A: Mat22) -> Mat22 ---

	// Solve A * x = b, where b is a column vector. This is more efficient
	// than computing the inverse in one-shot cases.
	@(link_name="b2Solve22")
	solve_22 :: proc(A: Mat22, b: Vec2) -> Vec2 ---

	// Does a fully contain b
	@(link_name="b2AABB_Contains")
	aabb_contains :: proc(a: AABB, b: AABB) -> bool ---

	// Get the center of the AABB.
	@(link_name="b2AABB_Center")
	aabb_center :: proc(a: AABB) -> Vec2 ---

	// Get the extents of the AABB (half-widths).
	@(link_name="b2AABB_Extents")
	aabb_extents :: proc(a: AABB) -> Vec2 ---

	// Union of two AABBs
	@(link_name="b2AABB_Union")
